...
def main(num_releases: int = 1) -> bool:
    ...
    for i, release in enumerate(releases, 1):
        ...
        extracted = download_and_extract_zip(
            zip_url, raw_data_dir, release_name=f"{title}_{release_date}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch USDA bee colony data using the ESMIS API")
    parser.add_argument(
        "-n",
        "--num-releases",
        type=int,
    ...
    parser.add_argument("--all", action="store_true", help="Download all available releases")
